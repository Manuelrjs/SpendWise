    ...
    <section className="mx-auto max-w-6xl space-y-6">
      ...
      <div className="grid gap-6 lg:grid-cols-[360px,1fr]">
        ...
        <div className="rounded-2xl border border-slate-200 bg-[var(--surface)] p-4 shadow-sm md:p-5">
          ...
          {cargando ? <p className="text-sm text-slate-600">Cargando categorías...</p> : categorias.length === 0 ? <p className="text-sm text-slate-600">Todavía no hay categorías registradas.</p> : <div className="space-y-3">{categorias.map((categoria) => <article key={categoria.id} className="rounded-xl border border-slate-200 p-4"><div className="mb-2 flex items-start justify-between gap-2"><h3 className="font-semibold text-slate-900">{categoria.icono ? `${categoria.icono} ` : ''}{categoria.nombre}</h3><span className={`inline-flex rounded-full px-2.5 py-1 text-xs font-medium ${categoria.activo ? 'bg-emerald-100 text-emerald-700' : 'bg-[var(--surface-3)] text-slate-600'}`}>{categoria.activo ? 'Activo' : 'Inactivo'}</span></div><p className="text-sm text-slate-600">Color: {categoria.color ?? '-'}</p><p className="text-sm text-slate-600">Orden: {categoria.orden ?? '-'}</p><p className="text-sm text-slate-500">Creado: {formatearFecha(categoria.creado_en)}</p><div className="mt-3 flex gap-2"><button type="button" onClick={() => cargarFormularioParaEditar(categoria)} className="flex-1 rounded-lg border border-slate-300 px-3 py-2 text-xs font-medium text-slate-700 hover:bg-[var(--surface-3)]">Editar</button><button type="button" onClick={() => cambiarEstadoCategoria(categoria, !categoria.activo)} className={`flex-1 rounded-lg border px-3 py-2 text-xs font-medium ${categoria.activo ? 'border-rose-200 text-rose-600 hover:bg-rose-50' : 'border-emerald-200 text-emerald-700 hover:bg-emerald-50'}`}>{categoria.activo ? 'Desactivar' : 'Reactivar'}</button></div></article>)}</div>}
        </div>
      </div>
    </section>
  );
