...
export default function Page() {
  ...
  const [accionCategoriaSugerida, setAccionCategoriaSugerida] = useState<'existente' | 'crear' | 'ignorar' | null>(null);
  const [creandoCategoria, setCreandoCategoria] = useState(false);
  const [modalCategoriaAbierto, setModalCategoriaAbierto] = useState(false);
  const [nuevaCategoriaNombre, setNuevaCategoriaNombre] = useState('');
  const [nuevaCategoriaIcono, setNuevaCategoriaIcono] = useState('');
  const [nuevaCategoriaColor, setNuevaCategoriaColor] = useState('');
  ...
  const categoriaSugeridaAplicada = useMemo(() => (sugerenciasIA?.categoria_id ? categorias.find((categoria) => categoria.id === sugerenciasIA.categoria_id) : null), [categorias, sugerenciasIA?.categoria_id]);
  ...
  useEffect(() => {
    if (comprobante && comprobante.type.startsWith('image/')) {
      ...
      setPreviewComprobanteUrl(objectUrl);
  ...
  useEffect(() => {
    ...
    if (!tokenCompartido) return;

    const cargarComprobanteCompartido = async () => {
      try {
        const respuesta = await fetch(`/api/compartir-temp/${tokenCompartido}`, { cache: 'no-store' });
        if (!respuesta.ok) {
    ...
    void cargarComprobanteCompartido();
  }, [searchParams]);
  useEffect(() => {
    if (!modalCategoriaAbierto) return;
  ...
  async function analizarComprobante() {
    ...
    if (comprobante.type === 'application/pdf') {
      setMensajeComprobante('La lectura automática de PDF se implementará en una fase posterior.');
      return;
    }
  ...
  async function crearYSeleccionarCategoria() {
    const nombreSugerido = nuevaCategoriaNombre.trim();
    if (!nombreSugerido) {
    ...
    const { data, error: errorInsert } = await supabase
      ...
      .single();
    if (errorInsert || !data) {
      setError('No se pudo crear la categoría. Probá nuevamente.');
      setCreandoCategoria(false);
      return;
    }
  ...
  function abrirModalCrearCategoria(nombreInicial?: string) {
    const nombreBase = typeof nombreInicial === 'string' ? nombreInicial.trim() : '';
    setNuevaCategoriaNombre(nombreBase);
    setNuevaCategoriaIcono(normalizarNombreCategoria(nombreBase) === 'impuestos' ? '🧾' : '');
    setNuevaCategoriaColor('');
  ...
  function manejarPegadoComprobante(event: ClipboardEvent<HTMLDivElement>) {
    ...
    if (!item) return setMensajeComprobante('Tu navegador no permite pegar imágenes aquí. Podés subirla desde galería o archivos.');
  ...
  async function pegarComprobanteDesdeBoton() {
    if (!navigator.clipboard?.read) {
      setMensajeComprobante('Tu navegador no permite pegar imágenes aquí. Podés subirla desde galería o archivos.');
      return;
    }
    try {
