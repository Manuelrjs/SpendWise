...
function esCandidatoEmisor(tipo: string | undefined) {
  ...
  return ['emisor', 'proveedor', 'vendedor', 'comercio', 'establecimiento', 'razon_social_emisora'].includes(tipo);
}

function elegirEstablecimientoPorCandidatos(sugerencias: DatosComprobanteSugeridos) {
  const candidatos = sugerencias.establecimiento_candidatos ?? [];
  const emisores = candidatos.filter((c) => esCandidatoEmisor(c.tipo));
  const priorizados = (emisores.length ? emisores : candidatos).slice().sort((a, b) => b.confianza - a.confianza);
  return priorizados[0]?.nombre ?? sugerencias.establecimiento;
}
