  ...
  return <section className="mx-auto max-w-[1440px] space-y-5 px-2 pb-6 md:px-4">
    ...
    <div className="rounded-2xl border border-slate-200 bg-[var(--surface)] p-4 shadow-sm md:p-5">
      ...
      <form onSubmit={guardarCuenta} className="grid grid-cols-1 gap-2 md:grid-cols-2 xl:grid-cols-6">
        ...
        <select value={formCuenta.persona_titular_id} onChange={(e) => setFormCuenta((p) => ({ ...p, persona_titular_id: e.target.value }))} className="w-full rounded-xl border border-slate-300 bg-[var(--surface)] px-3 py-2 text-sm"><option value="">Persona titular *</option>{personas.map((p) => <option key={p.id} value={p.id}>{nombrePersona(p)}</option>)}</select>
    ...
    <div className="space-y-3">
      ...
      {cargando ? <p className="text-sm text-slate-600">Cargando cuentas...</p> : cuentas.length === 0 ? <p className="rounded-xl border border-dashed border-slate-300 bg-[var(--surface)] p-4 text-sm text-slate-600">No hay cuentas de tarjeta registradas.</p> : <div className="grid gap-4 md:grid-cols-2 2xl:grid-cols-3">{cuentas.map((cuenta) => { const tarjetasCuenta = tarjetas.filter((x) => x.cuenta_tarjeta_id === cuenta.id); const sinConfiguracion = cuentaSinConfiguracion(cuenta); return <article key={cuenta.id} className="rounded-2xl border border-slate-200 bg-[var(--surface)] p-4 shadow-sm"><div className="mb-3 flex items-start justify-between gap-2"><div><h3 className="text-base font-semibold text-slate-900">{cuenta.icono_ui ? `${cuenta.icono_ui} ` : ''}{cuenta.nombre_cuenta}</h3><p className="text-xs text-slate-600">{cuenta.banco ?? '-'} · {cuenta.marca ?? '-'}</p></div><span className={`rounded-full px-2.5 py-1 text-[11px] font-semibold ${cuenta.activo ? 'bg-emerald-100 text-emerald-700' : 'bg-[var(--surface-3)] text-slate-700'}`}>{cuenta.activo ? 'Activa' : 'Inactiva'}</span></div><div className="space-y-1 text-xs text-slate-600"><p><span className="font-medium text-slate-700">Titular:</span> {nombrePersona(cuenta.persona_titular)}</p><p><span className="font-medium text-slate-700">Día cierre habitual:</span> {cuenta.dia_cierre_habitual ?? '-'}</p><p><span className="font-medium text-slate-700">Días hasta vencimiento:</span> {cuenta.dias_hasta_vencimiento ?? '-'}</p>{cuenta.observaciones && <p className="rounded-md bg-[var(--surface-2)] p-2">{cuenta.observaciones}</p>}</div>{sinConfiguracion && <div className="mt-3 rounded-xl border border-amber-200 bg-amber-50 p-3"><p className="text-xs font-semibold text-amber-800">Falta configuración de cierre/vencimiento</p><p className="mt-1 text-xs text-amber-700">Completá estos datos para que SpendFlow Planner pueda proyectar pagos automáticamente.</p><button type="button" onClick={() => editarCuenta(cuenta)} className="mt-2 rounded-lg border border-amber-300 bg-[var(--surface)] px-2.5 py-1.5 text-xs font-medium text-amber-800">Completar configuración</button></div>}<div className="my-3 flex flex-wrap gap-2"><button type="button" onClick={() => editarCuenta(cuenta)} className="rounded-lg border border-slate-300 px-2.5 py-1.5 text-xs text-slate-700">Editar cuenta</button><button type="button" onClick={() => cambiarEstadoCuenta(cuenta, !cuenta.activo)} className={`rounded-lg border px-2.5 py-1.5 text-xs ${cuenta.activo ? 'border-rose-200 text-rose-700' : 'border-emerald-200 text-emerald-700'}`}>{cuenta.activo ? 'Desactivar cuenta' : 'Reactivar cuenta'}</button><button type="button" onClick={() => prepararNuevaTarjeta(cuenta.id)} className="rounded-lg bg-emerald-600 px-2.5 py-1.5 text-xs font-semibold text-white">Agregar tarjeta física</button></div><div className="space-y-2 border-t border-slate-100 pt-3">{tarjetasCuenta.length === 0 ? <p className="rounded-xl border border-dashed border-slate-300 p-3 text-xs text-slate-500">Sin tarjetas físicas asociadas.</p> : tarjetasCuenta.map((tarjeta) => <div key={tarjeta.id} className="rounded-xl border border-slate-200 bg-[var(--surface-2)] p-3"><div className="mb-2 flex flex-wrap items-start justify-between gap-2"><p className="text-sm font-semibold text-slate-900">{tarjeta.alias || tarjeta.nombre_en_tarjeta || 'Tarjeta sin alias'}</p><span className={`rounded-full px-2 py-0.5 text-[11px] font-medium ${tarjeta.activo ? 'bg-emerald-100 text-emerald-700' : 'bg-[var(--surface-3)] text-slate-700'}`}>{tarjeta.activo ? 'Activa' : 'Inactiva'}</span></div><div className="space-y-1 text-xs text-slate-600"><p><span className="font-medium text-slate-700">Persona:</span> {nombrePersona(tarjeta.persona)}</p><p><span className="font-medium text-slate-700">Tipo:</span> {tarjeta.tipo}</p><p><span className="font-medium text-slate-700">Alias:</span> {tarjeta.alias ?? '-'}</p><p><span className="font-medium text-slate-700">Últimos 4:</span> {tarjeta.ultimos_4_digitos ?? '-'}</p></div><div className="mt-2 flex gap-2"><button type="button" onClick={() => editarTarjeta(tarjeta)} className="rounded-lg border border-slate-300 px-2 py-1 text-xs text-slate-700">Editar</button><button type="button" onClick={() => cambiarEstadoTarjeta(tarjeta, !tarjeta.activo)} className={`rounded-lg border px-2 py-1 text-xs ${tarjeta.activo ? 'border-rose-200 text-rose-700' : 'border-emerald-200 text-emerald-700'}`}>{tarjeta.activo ? 'Desactivar' : 'Reactivar'}</button></div></div>)}</div></article>; })}</div>}
    </div>

    {modalTarjetaAbierto && <div className="fixed inset-0 z-50 flex items-end justify-center bg-slate-900/50 p-0 md:items-center md:p-4"><div className="max-h-[90vh] w-full max-w-lg overflow-y-auto rounded-t-2xl bg-[var(--surface)] p-4 shadow-xl md:rounded-2xl md:p-5"><div className="mb-3 flex items-start justify-between gap-3"><div><h2 className="text-lg font-semibold">{tituloTarjeta}</h2><p className="text-xs text-slate-500">Asigná la tarjeta a esta cuenta de forma contextual.</p></div><button type="button" onClick={cerrarModalTarjeta} className="rounded-lg border border-slate-300 px-3 py-1.5 text-xs">Cerrar</button></div><form onSubmit={guardarTarjeta} className="space-y-3"><select value={formTarjeta.cuenta_tarjeta_id} onChange={(e) => setFormTarjeta((p) => ({ ...p, cuenta_tarjeta_id: e.target.value }))} className="w-full rounded-xl border border-slate-300 bg-[var(--surface)] px-3 py-2 text-sm"><option value="">Seleccionar cuenta *</option>{cuentas.map((c) => <option key={c.id} value={c.id}>{c.nombre_cuenta}</option>)}</select><select value={formTarjeta.persona_id} onChange={(e) => setFormTarjeta((p) => ({ ...p, persona_id: e.target.value }))} className="w-full rounded-xl border border-slate-300 bg-[var(--surface)] px-3 py-2 text-sm"><option value="">Seleccionar persona *</option>{personas.map((p) => <option key={p.id} value={p.id}>{nombrePersona(p)}</option>)}</select><select value={formTarjeta.tipo} onChange={(e) => setFormTarjeta((p) => ({ ...p, tipo: e.target.value as TipoTarjetaFisica }))} className="w-full rounded-xl border border-slate-300 bg-[var(--surface)] px-3 py-2 text-sm"><option value="titular">Titular</option><option value="adicional">Adicional</option></select><input value={formTarjeta.alias} onChange={(e) => setFormTarjeta((p) => ({ ...p, alias: e.target.value }))} placeholder="Alias" className="w-full rounded-xl border border-slate-300 px-3 py-2 text-sm" /><input value={formTarjeta.nombre_en_tarjeta} onChange={(e) => setFormTarjeta((p) => ({ ...p, nombre_en_tarjeta: e.target.value }))} placeholder="Nombre en tarjeta" className="w-full rounded-xl border border-slate-300 px-3 py-2 text-sm" /><input inputMode="numeric" value={formTarjeta.ultimos_4_digitos} onChange={(e) => setFormTarjeta((p) => ({ ...p, ultimos_4_digitos: e.target.value }))} placeholder="Últimos 4 dígitos" className="w-full rounded-xl border border-slate-300 px-3 py-2 text-sm" /><textarea value={formTarjeta.observaciones} onChange={(e) => setFormTarjeta((p) => ({ ...p, observaciones: e.target.value }))} rows={2} placeholder="Observaciones" className="w-full rounded-xl border border-slate-300 px-3 py-2 text-sm" />{errorTarjeta && <p className="text-sm text-rose-600">{errorTarjeta}</p>}<button disabled={guardandoTarjeta} type="submit" className="w-full rounded-xl bg-emerald-600 px-4 py-2.5 text-sm font-semibold text-white">{guardandoTarjeta ? 'Guardando...' : 'Guardar tarjeta física'}</button></form></div></div>}
  </section>;
}
