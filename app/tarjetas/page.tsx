  ...
  return <section className="mx-auto max-w-[1440px] space-y-5 px-2 pb-6 md:px-4">
    ...
    <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm md:p-5">
      ...
      <form onSubmit={guardarCuenta} className="grid grid-cols-1 gap-2 md:grid-cols-2 xl:grid-cols-6">
        ...
        <input inputMode="numeric" value={formCuenta.dia_cierre_habitual} onChange={(e) => setFormCuenta((p) => ({ ...p, dia_cierre_habitual: e.target.value }))} placeholder="Día cierre" className="w-full rounded-xl border border-slate-300 px-3 py-2 text-sm" />
        <input inputMode="numeric" value={formCuenta.dias_hasta_vencimiento} onChange={(e) => setFormCuenta((p) => ({ ...p, dias_hasta_vencimiento: e.target.value }))} placeholder="Días venc." className="w-full rounded-xl border border-slate-300 px-3 py-2 text-sm" />
