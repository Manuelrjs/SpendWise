-- Tarea 29: asociar datos operativos por grupo
...
-- Backfill de desarrollo (ejecutar luego de validar que existe al menos 1 grupo)
-- Si no hay grupos, esta sección no hace cambios.
with grupo_default as (select id from public.grupos order by creado_en asc limit 1)
update public.personas set grupo_id = (select id from grupo_default) where grupo_id is null and exists(select 1 from grupo_default);
with grupo_default as (select id from public.grupos order by creado_en asc limit 1)
update public.categorias set grupo_id = (select id from grupo_default) where grupo_id is null and exists(select 1 from grupo_default);
with grupo_default as (select id from public.grupos order by creado_en asc limit 1)
update public.medios_pago set grupo_id = (select id from grupo_default) where grupo_id is null and exists(select 1 from grupo_default);
with grupo_default as (select id from public.grupos order by creado_en asc limit 1)
update public.cuentas_tarjeta set grupo_id = (select id from grupo_default) where grupo_id is null and exists(select 1 from grupo_default);
with grupo_default as (select id from public.grupos order by creado_en asc limit 1)
update public.tarjetas_fisicas set grupo_id = (select id from grupo_default) where grupo_id is null and exists(select 1 from grupo_default);
with grupo_default as (select id from public.grupos order by creado_en asc limit 1)
update public.gastos set grupo_id = (select id from grupo_default) where grupo_id is null and exists(select 1 from grupo_default);
with grupo_default as (select id from public.grupos order by creado_en asc limit 1)
update public.cuotas_tarjeta set grupo_id = (select id from grupo_default) where grupo_id is null and exists(select 1 from grupo_default);
with grupo_default as (select id from public.grupos order by creado_en asc limit 1)
update public.calendario_tarjetas set grupo_id = (select id from grupo_default) where grupo_id is null and exists(select 1 from grupo_default);
with grupo_default as (select id from public.grupos order by creado_en asc limit 1)
update public.compras_cuotas_iniciales set grupo_id = (select id from grupo_default) where grupo_id is null and exists(select 1 from grupo_default);
with grupo_default as (select id from public.grupos order by creado_en asc limit 1)
update public.comprobantes set grupo_id = (select id from grupo_default) where grupo_id is null and exists(select 1 from grupo_default);
